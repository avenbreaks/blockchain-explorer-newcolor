defmodule Indexer.Fetcher.TokenBalanceTest do
  ...
  describe "init/3" do
    test "returns unfetched token balances" do
      ...
      assert TokenBalance.init([], &[&1 | &2], nil) == [
               {address_hash_bytes, token_contract_address_hash_bytes, 1000, "GTX-20", nil, 0}
             ]
    end
  end
  ...
  describe "run/3" do
    ...
    test "imports the given token balances" do
      ...
      assert TokenBalance.run(
               [{address_hash_bytes, token_contract_address_hash_bytes, block_number, "GTX-20", nil, 0}],
               nil
             ) == :ok
  ...
  describe "import_token_balances/1" do
    ...
    test "import the token balances and return :ok when there are multiple balances for the same address on the batch (GTX-20)" do
      contract = insert(:token)
    ...
    test "import the token balances and return :ok when there are multiple balances for the same address on the batch (GTX-1155)" do
      contract = insert(:token)
      contract2 = insert(:token)
      insert(:block, number: 19999)
