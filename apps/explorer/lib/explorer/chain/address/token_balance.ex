defmodule Explorer.Chain.Address.TokenBalance do
   ...
   *  `token_id` - The token_id of the transferred token (applicable for GTX-1155 and GTX-721 tokens)
  ...
  ignores the burn_address for tokens GTX-721 since the most tokens GTX-721 don't allow get the
  ...
  def unfetched_token_balances do
    from(
      ...
      where:
        ((tb.address_hash != ^@burn_address_hash and t.type == "GTX-721") or t.type == "GTX-20" or t.type == "GTX-1155") and
