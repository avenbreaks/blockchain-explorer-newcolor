defmodule Explorer.Account.Notifier.Notify do
  ...
  defp is_watched(%WatchlistAddress{} = address, %{type: type}, direction) do
    case {type, direction} do
      ...
      {"GTX-20", :incoming} -> address.watch_erc_20_input
      {"GTX-20", :outgoing} -> address.watch_erc_20_output
      {"GTX-721", :incoming} -> address.watch_erc_721_input
      {"GTX-721", :outgoing} -> address.watch_erc_721_output
      {"GTX-1155", :incoming} -> address.watch_erc_1155_input
      {"GTX-1155", :outgoing} -> address.watch_erc_1155_output
    end
  end
