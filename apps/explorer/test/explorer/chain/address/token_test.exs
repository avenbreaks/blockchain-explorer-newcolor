defmodule Explorer.Chain.Address.TokenTest do
  ...
  describe "list_address_tokens_with_balance/2" do
    test "returns tokens with number of transfers and balance value attached" do
      ...
      token =
        :token
        |> insert(name: "token-c", type: "GTX-721", decimals: 0, symbol: "TC")
    ...
    test "returns tokens ordered by type in reverse alphabetical order" do
      ...
      token =
        :token
        |> insert(name: nil, type: "GTX-721", decimals: nil, symbol: nil)
        |> Repo.preload(:contract_address)
      ...
      token2 =
        :token
        |> insert(name: "token-c", type: "GTX-20", decimals: 0, symbol: "TC")
        |> Repo.preload(:contract_address)
    ...
    test "returns tokens of same type by name in lowercase ascending" do
      ...
      token =
        :token
        |> insert(name: "atoken", type: "GTX-721", decimals: nil, symbol: nil)
        |> Repo.preload(:contract_address)
      ...
      token2 =
        :token
        |> insert(name: "1token-c", type: "GTX-721", decimals: 0, symbol: "TC")
        |> Repo.preload(:contract_address)
    ...
    test "does not return tokens with zero balance" do
      ...
      token =
        :token
        |> insert(name: "atoken", type: "GTX-721", decimals: 0, symbol: "AT")
        |> Repo.preload(:contract_address)
  ...
  describe "page_tokens/2" do
    ...
    test "add more conditions to the query when PagingOptions.key is not nil" do
      token1 = insert(:token, name: "token-a", type: "GTX-20", decimals: 0, symbol: "TA")

      token2 = insert(:token, name: "token-c", type: "GTX-721", decimals: 0, symbol: "TC")
    ...
    test "tokens with nil name come after other tokens of same type" do
      ...
      token2 = insert(:token, name: nil, type: "GTX-20", decimals: 0, symbol: "TC")
