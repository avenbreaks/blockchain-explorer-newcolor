defmodule BlockScoutWeb.Tokens.HolderView do
  ...
  ## Examples
    ...
    iex> token = build(:token, type: "GTX-20", decimals: Decimal.new(2))
    iex> BlockScoutWeb.Tokens.HolderView.format_token_balance_value(100000, nil, token)
    "1,000"

    iex> token = build(:token, type: "GTX-721")
    iex> BlockScoutWeb.Tokens.HolderView.format_token_balance_value(1, nil, token)
    1

  """
  def format_token_balance_value(value, _id, %Token{type: "GTX-20", decimals: decimals}) do
    format_according_to_decimals(value, decimals)
  end

  def format_token_balance_value(value, id, %Token{type: "GTX-1155", decimals: decimals}) do
