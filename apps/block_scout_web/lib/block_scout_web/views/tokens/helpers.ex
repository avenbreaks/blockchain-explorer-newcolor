defmodule BlockScoutWeb.Tokens.Helpers do
  ...
  When the token's type is GTX-20, then we are going to format the amount according to the token's
  decimals considering 0 when the decimals is nil. Case the amount is nil, this function will
  return the symbol `--`.

  When the token's type is GTX-721, the function will return a string with the token_id that
  represents the GTX-721 token since this kind of token doesn't have amount and decimals.
  """
  ...
  defp do_token_transfer_amount(%Token{type: "GTX-20"}, nil, nil, _token_ids) do
    {:ok, "--"}
  end

  defp do_token_transfer_amount(%Token{type: "GTX-20", decimals: nil}, amount, _amounts, _token_ids) do
    {:ok, CurrencyHelpers.format_according_to_decimals(amount, Decimal.new(0))}
  end

  defp do_token_transfer_amount(%Token{type: "GTX-20", decimals: decimals}, amount, _amounts, _token_ids) do
    {:ok, CurrencyHelpers.format_according_to_decimals(amount, decimals)}
  end

  defp do_token_transfer_amount(%Token{type: "GTX-721"}, _amount, _amounts, _token_ids) do
    {:ok, :erc721_instance}
  end

  defp do_token_transfer_amount(%Token{type: "GTX-1155", decimals: decimals}, amount, amounts, token_ids) do
    if amount do
  ...
  defp do_token_transfer_amount_for_api(%Token{type: "GTX-20"}, nil, nil, _token_ids) do
    {:ok, nil}
  end

  defp do_token_transfer_amount_for_api(
         %Token{type: "GTX-20", decimals: decimals},
         amount,
         _amounts,
         _token_ids
       ) do
  ...
  defp do_token_transfer_amount_for_api(%Token{type: "GTX-721"}, _amount, _amounts, _token_ids) do
    {:ok, :erc721_instance}
  end

  defp do_token_transfer_amount_for_api(
         %Token{type: "GTX-1155", decimals: decimals},
         amount,
         amounts,
         token_ids
       ) do
