defmodule BlockScoutWeb.Tokens.HolderViewTest do
  ...
  describe "format_token_balance_value/3" do
    test "formats according to token decimals when it's a GTX-20" do
      token = build(:token, type: "GTX-20", decimals: Decimal.new(2))
    ...
    test "returns the value when it's GTX-721" do
      token = build(:token, type: "GTX-721")
