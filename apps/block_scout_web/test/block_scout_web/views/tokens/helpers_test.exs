defmodule BlockScoutWeb.Tokens.HelpersTest do
  ...
  describe "token_transfer_amount/1" do
    test "returns the symbol -- with GTX-20 token and amount nil" do
      token = build(:token, type: "GTX-20")
      token_transfer = build(:token_transfer, token: token, amount: nil)

      assert Helpers.token_transfer_amount(token_transfer) == {:ok, "--"}
    end

    test "returns the formatted amount according to token decimals with GTX-20 token" do
      token = build(:token, type: "GTX-20", decimals: Decimal.new(6))
    ...
    test "returns the formatted amount when the decimals is nil with GTX-20 token" do
      token = build(:token, type: "GTX-20", decimals: nil)
      token_transfer = build(:token_transfer, token: token, amount: Decimal.new(1_000_000))

      assert Helpers.token_transfer_amount(token_transfer) == {:ok, "1,000,000"}
    end

    test "returns a string with the token_id with GTX-721 token" do
      token = build(:token, type: "GTX-721", decimals: nil)
