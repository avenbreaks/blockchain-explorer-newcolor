defmodule BlockScoutWeb.API.V2.AddressControllerTest do
  ...
  describe "/addresses/{address_hash}/token-transfers" do
    ...
    test "check token type filters", %{conn: conn} do
      ...
      erc_20_token = insert(:token, type: "GTX-20")
      ...
      erc_721_token = insert(:token, type: "GTX-721")
      ...
      erc_1155_token = insert(:token, type: "GTX-1155")
    ...
    test "type and direction filters at the same time", %{conn: conn} do
      ...
      filter = %{"type" => "GTX-721", "filter" => "from"}
      request = get(conn, "/api/v2/addresses/#{address.hash}/token-transfers", filter)
      assert response = json_response(request, 200)
      assert response["items"] == []
      assert response["next_page_params"] == nil

      filter = %{"type" => "GTX-721", "filter" => "to"}
      request = get(conn, "/api/v2/addresses/#{address.hash}/token-transfers", filter)
      assert response = json_response(request, 200)
      ...
      filter = %{"type" => "GTX-721,GTX-20", "filter" => "to"}
      request = get(conn, "/api/v2/addresses/#{address.hash}/token-transfers", filter)
      assert response = json_response(request, 200)
      ...
      filter = %{"type" => "GTX-721,GTX-20", "filter" => "from"}
      request = get(conn, "/api/v2/addresses/#{address.hash}/token-transfers", filter)
      assert response = json_response(request, 200)
  ...
  describe "/addresses/{address_hash}/tokens" do
    ...
    test "get tokens", %{conn: conn} do
      ...
      ctbs_erc_20 =
        for _ <- 0..50 do
          insert(:address_current_token_balance_with_token_id, address: address, token_type: "GTX-20", token_id: nil)
          |> Repo.preload([:token])
        end
      ...
      ctbs_erc_721 =
        for _ <- 0..50 do
          insert(:address_current_token_balance_with_token_id, address: address, token_type: "GTX-721", token_id: nil)
          |> Repo.preload([:token])
        end
        |> Enum.sort_by(fn x -> x.value end, :asc)
